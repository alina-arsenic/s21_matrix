...
START_TEST(test_s21_mult_matrix_6) {
    matrix_t A, B, C;
    int code = 0;
    code += s21_create_matrix(43, 5, &A);
    code += s21_create_matrix(5, 12, &B);
    code += s21_fill_matrix_random(&A, 100);
    code += s21_fill_matrix_random(&B, 100);
    code += s21_mult_matrix(&A, &B, &C);
    ck_assert_int_ne(0, code);
    s21_remove_matrix(&A);
    s21_remove_matrix(&B);
...
Suite *s21_mult_matrix_suite(void) {
    ...
    tcase_add_test(tc_s21_mult_matrix, test_s21_mult_matrix_6);
