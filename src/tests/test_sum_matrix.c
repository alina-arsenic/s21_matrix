    ...
    if (!code) {
        ...
        for (int i = 0; i < A.rows; i++) {
            for (int j = 0; j < A.columns; j++) {
                errors += !(fabs(B.matrix[i][j] + A.matrix[i][j] - C.matrix[i][j]) < 1e-6);
            }
        }
...
START_TEST(test_s21_sum_matrix_4) {
    ...
    code += s21_create_matrix(179, 179, &B);
    if (!code) {
        s21_fill_matrix_random(&A, 100);
        s21_fill_matrix_random(&B, 19);
...
Suite *s21_sum_matrix_suite(void) {
    ...
    tcase_add_test(tc_s21_sum_matrix, test_s21_sum_matrix_8);
